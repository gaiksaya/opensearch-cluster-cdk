...
| use50PercentHeap       | Optional    | boolean | Boolean flag to use 50% of physical memory as heap. Defaults to false.  e.g., `--context use50PercentHeap=true`                                                                                                                                                                                    |
| isInternal             | Optional    | boolean | Boolean flag to make network load balancer internal. Defaults to false.  e.g., `--context isInternal=true`                                                                                                                                                                              |
...
| storageVolumeType      | Optional    | string  | EBS volume type for all the nodes (data, ml, cluster manager). Defaults to gp2. See [node-config](./lib/opensearch-config/node-config.ts) for available options. E.g., `-c storageVolumeType=gp3`. For SSD based instance (i.e. i3 family), it is used for root volume configuration.                           |
